    ...
    <div className="min-h-screen bg-[var(--bg)] flex flex-col items-center justify-center p-8">
      <div className="w-full max-w-md">
        <div className="text-center mb-10">
          ...
          <h1 className="text-3xl font-syne font-extrabold text-white mb-3">
            Create Your Account
          </h1>
          <p className="text-sm text-[var(--muted)]">
            Venus AI is available exclusively to enterprise operators. Sign up to begin your access request.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-[var(--surface)] border border-[var(--border)] rounded-xl p-8 space-y-5">
          ...
          <div>
            <label className="block text-xs font-mono text-[var(--dim)] uppercase tracking-wider mb-2">Company</label>
            <input
              type="text"
              value={form.company}
        ...
        <div className="flex justify-center gap-2 mt-8">
          {['Gate 1', 'Gate 2', 'Gate 3', 'Gate 4'].map((g, i) => (
            <div key={g} className={`text-[10px] font-mono px-2 py-1 rounded ${i === 0 ? 'bg-[var(--indigo)] text-white' : 'bg-[var(--surface)] text-[var(--dim)] border border-[var(--border)]'}`}>
              {g}
            </div>
          ))}
        </div>
